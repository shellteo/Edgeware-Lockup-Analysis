<template>
    <div style="padding: 20px;" >
      <div>
        <h1>Edgeware Lockup Analysis</h1>
      </div>
      <el-tabs v-model="activeName">
        <el-tab-pane label="概览信息" name="chart">  
          <div id="myChart" :style="{width: '100%', height: '500px'}"></div>
          <div id="pieChart" :style="{width: '100%', height: '300px'}"></div>
        </el-tab-pane>
        <el-tab-pane label="大户锁仓" name="table">
  ...
  methods: {
    async getData() {
      ...
      const loading = this.$loading({
        lock: true,
        text: 'Loading data from the Ethereum blockchain, will take a long time, please wait...',
